...
export function ExpensesPieChart({ className }: ExpensesPieChartProps) {
  ...
  const totalMonthlyExpenses = chartData.reduce((sum, item) => sum + item.amount, 0)
    ...
    <Card className={`flex flex-col ${className || ''}`}>
      ...
      <CardFooter className="flex-col gap-2 text-sm">
        <div className="flex items-center gap-2 leading-none font-medium">
          Total Monthly Expenses: ${totalMonthlyExpenses.toLocaleString('en-US', { 
            minimumFractionDigits: 2, 
            maximumFractionDigits: 2 
          })}
        </div>
        <div className="text-muted-foreground leading-none">
          Showing breakdown of {chartData.length} expense{chartData.length !== 1 ? 's' : ''}
        </div>
      </CardFooter>
    </Card>
  )
